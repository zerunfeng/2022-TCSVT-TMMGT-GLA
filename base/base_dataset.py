...
def choose_or_pad_to_len(features,
  ...
  if seq_len < max_tokens:
    tensor[:seq_len, :] = features
    tensor_t[:seq_len] = features_t
    tensor_ind[:seq_len] = 1
  # choose
  else:
    idxs = np.round(np.linspace(0, seq_len - 1, max_tokens)).astype(np.int32)
    tensor = features[idxs]
    tensor_t = features_t[idxs]
    tensor_ind[:] = 1
    ...
    if self.reading_from == "mult_h5":
      if self.loaded_in_ram:
        ...
        with h5py.File(dataset_file_path, "r") as dataset_file:
          ...
          # Downsampling the rate of rgb to match scene, s3d and vggish
          features['rgb'] = features['rgb'][0:-1:5]
          features_t['rgb'] = features_t['rgb'][0:-1:5]
