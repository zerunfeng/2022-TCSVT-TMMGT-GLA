...
class BaseTrainer:
  ...
  def train(self):
    ...
    for epoch in range(self.start_epoch, self.epochs + 1):
          ...
          for dataset_name, dataset_metrics in value.items():
            for metric_type, metric_dict in dataset_metrics.items():
              for metric_name, metric_value in metric_dict.items():
                if metric_name in ['R1', 'R5', 'R10']:
                  rsum = rsum + metric_value
      ...
      for metric_name, metric_value in log.items():
        temp = metric_name.split('/')
        if len(metric_name.split('/')) > 1:
          if metric_name.split('/')[2] == 'rsum':
            rsum_all = rsum_all + metric_value
      ...
      logger.info(' {:15s}: {}'.format('The sum of recall is ', rsum_all))
  ...
  def evaluate(self):
    """Final evaluation."""
    sets = 'final_eval'
    ckpt_path = self.config.save_dir / 'trained_best_model.pth'
  ...
  def _save_checkpoint(self, epoch, save_best=False):
    ...
    if save_best:
      ...
      best_path = str(self.checkpoint_dir / 'trained_best_model.pth')
