...
from model.model import sharded_cross_view_inner_product_vlad
import numpy as np
...
class Trainer(BaseTrainer):
  ...
  def _train_epoch(self, epoch, val_first=False):
    # There is no training at epoch 0, only evaluation
    if epoch == 0 and val_first:
    ...
    i = 0
    while self.data_loaders["train_sets"][i].until_epoch < epoch:  # the condition never satisfied
      i += 1
  ...
  def _get_embeddings(self, val_loader):
    ...
    with torch.no_grad():
      ...
      for batch_idx, minibatch in enumerate(val_loader):
        ...
        vid_vlad_list.append(output["vid_vlad"])
        text_vlad_list.append(output["text_vlad"])
        for idx, mod in enumerate(self.modalities):
      ...
      vid_vlad = torch.cat(vid_vlad_list, 0)
      text_vlad = torch.cat(text_vlad_list, 0)
      for idx, mod in enumerate(self.modalities):
    ...
    with torch.no_grad():
      ...
      for dataset_name, embds in loaders_embds.items():
        ...
        cross_view_conf_matrix = sharded_cross_view_inner_product_vlad(
            ...
            subspaces=self.modalities,
            vid_vlad=embds["vid_vlad"],
            text_vlad=embds["text_vlad"],
