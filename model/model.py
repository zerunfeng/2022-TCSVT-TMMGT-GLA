...
from model.bert import MultiModalGraphTrans
...
class CENet(BaseModel):
  ...
  def __init__(self,
    ...
    # If Bert architecture is employed for video
      ...
      self.vid_bert = MultiModalGraphTrans(vid_bert_config)
    ...
    self.text_com = ReduceDim(text_dim, same_dim)
    ...
    self.vlad = NetVLAD(vlad_clu_num, same_dim)
  ...
  def forward(self,
    ...
    ind = {}
    for mod in self.modalities:  # set the indicator of different modal features
      ind[mod] = th.max(experts_feats_ind[mod], 1)[0]  # indication of the existence
    pooled_experts = {}
    ...
    if self.txt_agg[:4] in ['bert']:
      ...
      input_ids = th.stack(input_ids_list, dim=1).to(device)  # the id of each word(containing cls and sep)
      ...
      text_word = last_layer[:, 1:]
      text_word = self.text_com(text_word)
      attention_mask_word = attention_mask[:, 1:].unsqueeze(2).expand(-1, -1, self.same_dim).byte()
      attention_mask_word_vlad = attention_mask[:, 1:].byte()
      # attention_mask_word_arr = attention_mask_word.detach().cpu().numpy()
      text_word = text_word.masked_fill(attention_mask_word == 0, 0)
    ...
    _, length, dim = experts_feats[mod].shape
    ...
    if self.vid_cont == 'bert':
      ...
      # [CLS] token
      cls_size = (b, m,)
      input_ids_list.append(th.full(cls_size, 0, dtype=th.long).to(device))
      token_type_ids_list.append(th.full(cls_size, 0, dtype=th.long).to(device))
      position_ids_list.append(th.full(cls_size, 0, dtype=th.long).to(device))
      # features_list.append(
      #     th.full((b, m, self.same_dim), 0, dtype=th.float).to(device))
      mod_mask_list = []
      features_indi_list = []
      for _, modality in enumerate(self.modalities):
        mod_mask_list.append(ind[modality].long().to(device))
        features_indi_list.append(maxp_experts[modality].to(device))
      mod_mask = th.stack(mod_mask_list, dim=1).to(device)
      features_list.append(th.stack(features_indi_list, dim=1).to(device))
      attention_mask_list.append(mod_mask)
      ...
      if self.vid_inp in ['agg', 'both', 'all']:
        ...
        for frame_id in range(length):
          features_indi_list = []
          input_ids_indi_list = []
          token_type_ids_indi_list = []
          position_ids_indi_list = []
          attention_mask_indi_list = []
          for idx, modality in enumerate(self.modalities):
            input_ids_indi_list.append(th.full(ids_size, 6, dtype=th.long).to(device))
            token_type_ids_indi_list.append(th.full(ids_size, idx + 1, dtype=th.long).to(device))
            attention_mask_indi_list.append(
              experts_feats_ind[modality][:, frame_id].to(dtype=th.long))
            if self.pos_enc == 'ordr':
              position_ids_indi_list.append(
                  th.full(ids_size, frame_id + 1, dtype=th.long).to(device))
            elif self.pos_enc == 'tint':
              position_ids_indi_list.append(experts_feats_t[modality][:, frame_id])
            if modality in ['face', 'speech', 'ocr']:
              # temp = maxp_experts[modality].cpu().detach().numpy()
              # features_indi_list.append(maxp_experts[modality])
              features_indi_list.append(experts_feats[modality][:, frame_id, :])
            else:
              features_indi_list.append(experts_feats[modality][:, frame_id, :])
          features_list.append(th.stack(features_indi_list, dim=1).to(device))
          input_ids_list.append(th.stack(input_ids_indi_list, dim=1).to(device))
          token_type_ids_list.append(th.stack(token_type_ids_indi_list, dim=1).to(device))
          position_ids_list.append(th.stack(position_ids_indi_list, dim=1).to(device))
          attention_mask_list.append(th.stack(attention_mask_indi_list, dim=1).to(device))
      features = th.stack(features_list, dim=2).to(self.device)
      input_ids = th.stack(input_ids_list, dim=2).to(self.device)
      token_type_ids = th.stack(token_type_ids_list, dim=2).to(self.device)
      position_ids = th.stack(position_ids_list, dim=2).to(self.device)
      attention_mask = th.stack(attention_mask_list, dim=2).to(self.device)
      ...
      vid_bert_output = self.vid_bert(input_ids,
                                      ...
                                      position_ids=position_ids,
                                      features=features)
      vid_seg = vid_bert_output[:, :, 1:, :]
      b, m, seq, dim = vid_seg.shape
      vid_seg = vid_seg.reshape(b, m*seq, dim)
      attention_mask_seq = attention_mask[:, :, 1:].reshape(b, m*seq)
      attention_mask_vid_vlad = attention_mask_seq.byte()
      attention_mask_seq = attention_mask_seq.unsqueeze(2).expand(-1, -1, self.same_dim).byte()

      # attention_mask_word_arr = attention_mask_seq.detach().cpu().numpy()
      vid_seg = vid_seg.masked_fill(attention_mask_seq == 0, 0)

      for idx, modality in enumerate(self.modalities):
        experts[modality] = vid_bert_output[:, idx, 0, :]

    text_word_vlad, text_assign = self.vlad(text_word, attention_mask_word_vlad)
    vid_seg_vlad, vid_assign = self.vlad(vid_seg, attention_mask_vid_vlad)
    vid_embd = None
    if self.vid_wgh == 'nrm':
    ...
    if out == 'conf':  # Output confusion matrix
      cross_view_conf_matrix = sharded_cross_view_inner_product_vlad(
          vid_embds=experts,
          text_embds=text_embd,
          vid_weights=vid_weights,
          text_weights=text_weights,
          subspaces=self.modalities,
          text_vlad=text_word_vlad,
          vid_vlad=vid_seg_vlad,
      ...
      return {
          ...
          'vid_assign': vid_assign,
          'text_mask': attention_mask_word_vlad,
          'vid_mask': attention_mask_vid_vlad
      }
...
def sharded_cross_view_inner_product_vlad(vid_embds,
                                          text_embds,
                                          vid_weights,
                                          text_weights,
                                          subspaces,
  ...
  sims = (sims + th.matmul(text_vlad, vid_vlad.t())) / 2
